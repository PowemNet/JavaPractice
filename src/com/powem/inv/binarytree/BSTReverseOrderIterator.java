...
// Create an Iterator class called BSTReverseOrderIterator which can be used to iterate over a Binary search tree(BST) in
// reverse order.
...
//then next() calls would yield 10, then 11, then 9, then 8, then 3.


import java.util.Stack;

public class BSTReverseOrderIterator {

  Stack<Integer> nodes;

  public BSTReverseOrderIterator(Integer[] items) {
    TreeNode root = createTreeNode(items);
    this.nodes = new Stack<Integer>();
    this.flatten(root);
  }
